...
def fit_plane_svd(v):
    ...
    v = v - mean
    c = torch.matmul(v.T, v)
    _, _, eigv = torch.linalg.svd(c)
    normal = eigv[2]
    return mean, normal
...
def bbox_3d(v, v_max=None, expand=1.):
    """calculate 3D bounding-box in Xiu's convention. The input is
    either a set of 3D points or extreme points [v_min, v_max]
    """
    if v_max is None:
        v_min = torch.min(v, dim=0)[0]
        v_max = torch.max(v, dim=0)[0]
...
def spherical_uniform_sampling(N_sample) -> torch.Tensor:
    u = torch.rand(N_sample)
    v = torch.rand(N_sample)
...
def cartesian_to_spherical(
    ...
    norm_xy = torch.norm(xyz[..., :2], dim=-1)

    xyz = F.normalize(xyz, dim=-1)
    x, y, z = xyz[..., 0], xyz[..., 1], xyz[..., 2]
    theta = torch.acos(z)
    phi = torch.where(norm_xy > eps, torch.atan2(x, y), torch.tensor([0.]))
    return torch.stack([theta, phi], dim=-1)
...
def ray_cube_intersection(ray, box, inf=1e8):
    ...
    Args:
        ...
        box: tensor of shape `[6,3]`, the bounding-box. Using Xiu's
    ...
    """
    near = torch.full([ray.shape[0], 6], inf,device=ray.device)
    far = torch.full([ray.shape[0], 6], -1.,device=ray.device)
